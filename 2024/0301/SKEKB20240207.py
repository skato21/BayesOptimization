...
def getInitSetting( values ) :
    ...
    return [repetition, dataN, iterN, WaitTime,
            ...
            method, UCB, logEI, beta, randomvalue, bestvalue, enqueueData]
...
def setValueX_PV(x_name_list, newX) :
    print(len(x_name_list),newX)
    for i in range(0, len(x_name_list) ) :
        caput( x_name_list[i], newX[i] )
    return
...
#   ベイズ最適化の1step
# -----------------------------------------------------------
def optimizationOneStep(study,x_name_list,y_name_list,y_alias_list,x_min_max_list,x_step_list,iter_i,WaitTime,dataN,limitationText,functionText) :
    ...
    while data_i < dataN :
        ...
        if limitationText != '' :
            ans = getValueY_PV(y_name_list, y_alias_list, limitationText) 
            limit_bool = ans[0]
            print(limit_bool)
...
def graph_view(X_values_list, Y_values_list, x_min_max_list) :
    ...
    ax1.set_ylabel("Applied Current (A)")
...
def saveSetting( values, fname ) :
    ...
    method,UCB ,logEI, beta, randomvalue, bestvalue, enqueueData] = getInitSetting( values )
    ...
    with open( fname, 'w') as f:
        ...
        print(f"limitation = {limitationText}", file=f)
        print('', file=f)
        ...
        if bestvalue == True :
            print("Initialization = enqueue", file=f)
            print("#Initialization = param_init", file=f)
        elif randomvalue == True:
            print("#Initialization = enqueue", file=f)
            print("Initialization = param_init", file=f)
        else :
            print("#Initialization = enqueue", file=f)
...
def readSetting(window, fname ) :
    ...
    if os.path.exists(fname) :
        ...
        limitationText = str(config_ini.get("PV", "limitation"))
        ...
        window['limitation'].update(limitationText)
...
if __name__ == '__main__':
    ...
    layout1 = [
        ...
        #[sg.Multiline("", size=(80, 4),visible = False , key='limitation') ],#windowからは見えなくしたコード
        ...
        [ sg.Multiline("", size=(110, 18), key='log')],
    ]
    ...
    while True: #ループに入る
        ...
        #study.set_user_attr("TEST:X0", [0.1, 0.5])
        ...
        if event == "start":
            ...
            study = optuna.create_study(
                ...
                sampler=Hitohudebayes.HitohudebayesSampler(x_name_list,x_min_max_list,x_weight_list,UCB,logEI,beta,n_startup_trials=10),
                study_name="{}".format(filename),
                storage="sqlite:///GUItest.db",
            )
            ...
            filename = "./log_" + now.strftime("%Y_%m_%d_%H_%M_%S") + ".csv"
            f = open(filename, "a")
            writer = csv.writer(f)
        ...
        if mode == 'run' :
            ...
            if method == 'Baysian' : #------- Baysian optimaze ----------------
                ...
                [X_vals,Y_val,optimization_text] = optimizationOneStep(study,x_name_list,y_name_list,y_alias_list,x_min_max_list,x_step_list,iter_i,WaitTime,dataN,limitationText,functionText)
                ...
                info_text = 'best y = {} at x = {} '.format(bestY_value, bestXset)
                log_text += info_text +'\n'
                ...
                if iter_i == iterN :
                    ...
                    f.close()
                    fig.savefig("./GUIplot" + now.strftime("%Y_%m_%d_%H_%M_%S") + ".png")