...
def getInitSetting( values ) :
    ...
    functionText = values['functionText']
    ...
    for i in range(0, maxN_th) :
        ...
        threshold = values['threshold{}'.format(i)]
        if name != '' and alias != '' :
    ...
    lm_alias_list = []
    lossmonitor_list = []
    for i in range(0, maxN_lm) :
        name = values['name_lm{}'.format(i)]
        alias = values['alias_lm{}'.format(i)]
        lossmonitor = values['lossmonitor{}'.format(i)]
        if name != '' and alias != '' :
            lm_name_list.append(name)
    ...
    while data_i < dataN :
        ...
        if len(threshold_list) != 0 :
            ...
            for i in range (len(threshold_list)):
                ...
                #しきい値が0のときは待機し続ける
                while threshold_list[i] == 0 :
                    time.sleep(3)
                    
                ans = getValueY_th_PV(y_name_list, y_alias_list,th_name_list, th_alias_list, threshold_list[i]) 
                limit_bool = ans[0]
                ...
                if limit_bool == False :
                    break
                elif not (type(limit_bool) is bool) :
                    limit_bool = True
                    print(limit_bool)
        else :
            limit_bool = True
...
def graph_view(X_values_list, Y_values_list, x_min_max_list) :
    ...
    ax1.set_ylabel("param_1",color = "C0")
    ax1.tick_params(axis = "y", labelcolor = "C0")
    
    plt.text(1.08, 1.71, 'param_2', va='center', ha='left', rotation='vertical', 
...
def saveSetting( values, fname ) :
    ...
    with open( fname, 'w') as f:
        ...
        print(f"nlm = {len(lm_name_list)}", file=f)
        ...
        for i in range (len(th_name_list)):
            ...
            print(f"limitation = {threshold_list[i]}", file=f)
            print('', file=f)
            
        for i in range (len(lm_name_list)):
            print(f"[PV_lm{i}]", file=f)
            print(f"name = {lm_name_list[i]}", file=f)
            print(f"alias = {lm_alias_list[i]}", file=f)
            print(f"limitation = {lossmonitor_list[i]}", file=f)
            print('', file=f)
    ...
    if os.path.exists(fname) :
        ...
        for i in range(0, nth ) :
            if i < len(th_name_list) :
                ...
                window['threshold{}'.format(i)].update(threshold_list[i])
            else :
                window['name_th{}'.format(i)].update('')
                window['alias_th{}'.format(i)].update('')
                window['threshold{}'.format(i)].update('')
                
        for i in range(0, nlm ) :
            if i < len(lm_name_list) :
                window['name_lm{}'.format(i)].update(lm_name_list[i])
                window['alias_lm{}'.format(i)].update(lm_alias_list[i])
                window['lossmonitor{}'.format(i)].update(lossmonitor_list[i])
            else :
                window['name_lm{}'.format(i)].update('')
                window['alias_lm{}'.format(i)].update('')
                window['lossmonitor{}'.format(i)].update('')

        window['YsettingText'].update(YsettingText[:-1])
        if len(YsettingText) > 5 :
            window['YTextFrame'].update(visible=True)

        window['functionText'].update(functionText)
...
if __name__ == '__main__':
    ...
    for i in range(0, maxN_th) :
        col_th_setting.append( 
            ...
            sg.Text("limitation :"), sg.InputText('', size=(65, 1), key='threshold{}'.format(i)), 
            ])
    ...
    maxN_lm = 3
    for i in range(0, maxN_lm) :
        col_lm_setting.append( 
            [ sg.InputText('', size=(30, 1), key='name_lm{}'.format(i)),
            sg.InputText('lm{}'.format(i), size=(8, 1), key='alias_lm{}'.format(i) ),
            sg.Text("limitation :"), sg.InputText('', size=(65, 1), key='lossmonitor{}'.format(i)), 
            ])
    ...
    layout1 = [
        ...
        [sg.Frame('Evaluate function', [[sg.Multiline("", size=(40, 2), key='functionText')]])]
        ])],
        [sg.Frame('Threshold settings', [[sg.Column(col_th_setting)]])],
        [sg.Frame('Loss monitor settings', [[sg.Column(col_lm_setting)]])],
    ...
    window['functionText'].update('y0')
    
    #window['name_th0'].update('TEST:th0')
    #window['threshold0'].update('th0 > 0.01')
    
    #window['name_lm0'].update('TEST:lm0')
    #window['lossmonitor0'].update('lm0 > 0.01')
    
    #window['name_lm1'].update('TEST:lm1')
    #window['lossmonitor1'].update('lm1 > 0.01')
    
    #window['name_lm2'].update('TEST:lm2')
    #window['lossmonitor2'].update('lm2 > 0.01')
    ...
    while True: #ループに入る
        ...
        if event == "start":
            ...
            study = optuna.create_study(
                ...
                storage="sqlite:///SKEKB20240301.db",
                #制限あり
        ...
        if event == 'setBestFinish' :
            ...
            if len(bestXset) > 1 :
                setValueX_PV(x_name_list,x_step_list ,list(study.best_params.values()) , Xold, WaitTime,iter_i, iterN) #最適値をセットする
