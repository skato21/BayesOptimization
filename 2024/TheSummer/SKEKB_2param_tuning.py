...
import configparser
import TheSummer.safeopt_code_basic as so
import subprocess
import select
...
import optuna
from TheSummer.safeopt_code_basic import constrained_candidates_func
    ...
    for j in range(1, max(nstep) + 1):
        for k in range(len(x_name_list)):
            if j <= nstep[k]:
                ...
                #while caget(th_name_list[0]) < float(0.1) :
                #    print("wait for gate opening...")
                #    time.sleep(1)
...
def optimizationOneStep(study,x_name_list,y_name_list,y_alias_list,x_min_max_list,x_step_list,iter_i,WaitTime,dataN,threshold_list,functionText) :
        ...
        if len(threshold_list) != 0 :
            ...
            if ans[1] == False :
                ...
                for i in range(len(lm_name_list)):
                    c_temp_val = float(caget(lm_name_list[i]))
    ...
    c_vals = np.max(c_temp_list_list_np, axis=0).tolist() #行列の列方向に中央値をとるその後ndarrayをlistに変換
    trial.set_user_attr("constraints", c_vals) #それぞれの制限のmaxを"constraints"に引き渡す
    print(c_vals,"c_vals")
    ...
    return [X,Y_val,log_text, c_vals]
...
def readSetting(args) :
    ...
    if os.path.exists(args) :
        ...
        if config_ini.get("PV", "acquisition_type_obj") == "EI":
            EI_obj = True
            UCB_obj = False
        elif config_ini.get("PV", "acquisition_type_obj") == "UCB":
            EI_obj = False
            UCB_obj = True
        else : print("The acquisition function is not correctly selected.")
        
        if config_ini.get("PV", "acquisition_type_con") == "EI":
            EI_con = True
            UCB_con = False
        elif config_ini.get("PV", "acquisition_type_con") == "UCB":
            EI_con = False
            UCB_con = True
        else : print("The constraint acquisition function is not correctly selected.")
        
        beta_obj = float(config_ini.get("PV", "beta_obj"))
        beta_con = float(config_ini.get("PV", "beta_con"))
        constraint_threshold = float(config_ini.get("PV", "constraint_threshold"))
    ...
    UCB_obj, EI_obj, UCB_con, EI_con, beta_obj, beta_con, constraint_threshold,
    randomvalue, gridvalue, bestvalue, enqueueData]
...
if __name__ == '__main__':
    ...
    lm_name_list, lm_alias_list, lossmonitor_list, functionText,
    UCB_obj, EI_obj, UCB_con, EI_con, beta_obj, beta_con, constraint_threshold,
    randomvalue, gridvalue, bestvalue, enqueueData] = readSetting(args[1])
    print(lm_name_list)
    
    #if repetition < 1 : repetition = 1
    if repetition > 50 : repetition = 50
    t = 1000/repetition #timeoutして自動更新する時間を決めるrepetition
    newX = [] 
    ...
    study = optuna.create_study(
        ...
        storage="sqlite:///SKEKB2024.db",
        #制限あり
        #sampler = so.SafeOptSampler(x_name_list,x_min_max_list,x_weight_list,UCB_obj,EI_obj,UCB_con, EI_con, beta_obj, beta_con, constraint_threshold, n_startup_trials = 5),
        
        sampler = so.SafeOptSampler(
                    x_name_list=x_name_list,
                    x_min_max_list=x_min_max_list,
                    x_weight_list=x_weight_list,
                    UCB_obj=UCB_obj,
                    EI_obj=EI_obj,
                    UCB_con=UCB_con,
                    EI_con=EI_con,
                    ...
                    candidates_func=constrained_candidates_func,
                    n_startup_trials=1,
                )
        
        #制限なし
        #sampler=Hitohudebayes.HitohudebayesSampler(x_name_list,x_min_max_list,x_weight_list,UCB,EI,beta,n_startup_trials = 1,),
    )
    ...
    if randomvalue == True:
        ...
        # 各次元の中点を計算
        center_points = [(x_min_max[0] + x_min_max[1]) / 2 for x_min_max in x_min_max_list]
        
        # 各次元の1/10の距離を計算
        delta_points = [(x_min_max[1] - x_min_max[0]) / 10 for x_min_max in x_min_max_list]

        # 中心点をenqueue
        center_dict = {x_name_list[i]: center_points[i] for i in range(len(x_name_list))}
        study.enqueue_trial(center_dict)

        # 各次元について中心から1/10の距離にある点をenqueue
        for i in range(len(x_name_list)):
            # 中心から +delta の点
            plus_delta_dict = center_dict.copy()
            plus_delta_dict[x_name_list[i]] = center_points[i] + delta_points[i]
            study.enqueue_trial(plus_delta_dict)

            # 中心から -delta の点
            minus_delta_dict = center_dict.copy()
            minus_delta_dict[x_name_list[i]] = center_points[i] - delta_points[i]
            study.enqueue_trial(minus_delta_dict)
    ...
    while iter_i < iterN :
        ...
        [X_vals,Y_val,optimization_text,c_vals] = optimizationOneStep(study,x_name_list,y_name_list,y_alias_list,x_min_max_list,x_step_list,iter_i,WaitTime,dataN,threshold_list,functionText)
        iter_i_list.append(iter_i)
        X_values_list.append(X_vals)
        Y_values_list.append(Y_val)
        c_values_list.append(c_vals)
        ...
        ax1.scatter(iter_i_list[:-1], Y_values_list[:-1], color="tab:blue", s=100)  # 以前の点を描写
        ax1.scatter(iter_i_list[-1], Y_values_list[-1], color="tab:blue", s=150, marker='*')  # 最新の点を星マークで描写

        ax1.plot(iter_i_list, best_value_list, color="tab:orange")
        ax1.scatter(iter_i_list[:-1], best_value_list[:-1], color="tab:orange", s=100)
        ax1.scatter(iter_i_list[-1], best_value_list[-1], color="tab:orange", s=150, marker='*')  # 最新の最良点を星マークで描写
        ...
        sc = ax2.scatter(transposed_X_values_list_np[0][:-1], transposed_X_values_list_np[1][:-1], c=Y_values_list[:-1], cmap="coolwarm", marker='o', s=100, vmin=0, vmax=100)
        # 最新の点を星マークで描写
        ax2.scatter(transposed_X_values_list_np[0][-1], transposed_X_values_list_np[1][-1], c=Y_values_list[-1], cmap="coolwarm", marker='*', s=150, vmin=0, vmax=100)
        ...
        with open(filename,"a") as f:
            writer = csv.writer(f)
            writer.writerow(
            [iter_i, Y_val, study.best_value] + X_vals + list(study.best_params.values()) + c_vals
            )
