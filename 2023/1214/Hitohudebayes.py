...
def logei_candidates_func(
    ...
    acqf_lib = {} #獲得関数を入れる辞書を作成
    Hitohude_lib = {} #一筆書きの可否を決める辞書を作成
    
    acqf1 = LogExpectedImprovement(
        model=model,
        best_f=best_f
    )
    acqf_lib["logEI"]= acqf1 #辞書にEIを追加
    
    acqf2 = UpperConfidenceBound(
        model=model,
        beta=float(config_ini.get("PV", "beta"))
    )
    acqf_lib["UCB"]= acqf2 #辞書にUCBを追加
    
    acqf = acqf_lib[config_ini.get("PV", "aquisition_function")] #iniから獲得関数を選択する
    
    Hitohude_lib["No_Hitohude"] = acqf #辞書にそのままの獲得関数を追加
    
    proximal_weights_modified = torch.ones(nxr)

    proximal_weights = torch.tensor(weights_list)
    
    for i in range (nxr):
            proximal_weights_modified[i] = max(proximal_weights[i], 1/min((abs(Xcopy[i]-xrange[i][0] + pow(10,-10)),abs(Xcopy[i]-xrange[i][1] + pow(10,-10))))) #壁に寄ったらsigmaを大きくするように変更
    print(proximal_weights_modified)

    acqf_proximal = proximal.ProximalAcquisitionFunction(acqf, proximal_weights)
    ...
    candidates, _ = optimize_acqf(
        ...
        acq_function=Hitohude_lib[config_ini.get("PV", "Hitohude")],
